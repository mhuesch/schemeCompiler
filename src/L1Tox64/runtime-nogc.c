/*
 * EECS 322 Compiler Construction
 * Northwestern University
 *
 * L1 language runtime, with no
 * garbage collector functionality.
 *
 * Use the "-m64" GCC flag during compilation!
 */

#include <string.h>
#include <stdlib.h>
#include <stdio.h>

void print_content(void** in, int depth) {
  if (depth >= 4) {
    printf("...");
    return;
  }
  long x = (long) in;
  if (x&1) {
    printf("%ld",x>>1);
  } else {
    long size= *((long*)in);
    void** data = in+1;
    int i;
    printf("{s:%ld", size);
    for (i=0;i<size;i++) {
      printf(", ");
      print_content(*data,depth+1);
      data++;
    }
    printf("}");
  }
}

int print(void* l) {
  print_content(l,0);
  printf("\n");
  return 1;
}


#define HEAP_SIZE 1048576  // one megabyte

void** heap;
void** allocptr;
int words_allocated=0;

void* allocate(long fw_size, void *fw_fill) {
  long size = fw_size >> 1;
  void** ret = (void**)allocptr;
  int i;

  if (!(fw_size&1)) {
    printf("allocate called with size input that was not an encoded integer, %ld\n",
           fw_size);
    exit(-1);
  }
  if (size < 0) {
    printf("allocate called with size of %ld\n",size);
    exit(-1);
  }

  allocptr+=(size+1);
  words_allocated+=(size+1);

  if (words_allocated < HEAP_SIZE) {
    *((int*)ret)=size;
    void** data = ret+1;
    for (i=0;i<size;i++) {
      *data = fw_fill;
      data++;
    }
    return ret;
  }
  else {
    printf("out of memory");
    exit(-1);
  }
}

/* reporting array dereference errors: */

int print_error(int* array, int fw_x) {
  printf("attempted to use position %i in an array that only has %i positions\n",
		 fw_x>>1, *array);
  exit(0);
}

int main() {
  heap=(void*)malloc(HEAP_SIZE*sizeof(void*));
  if (!heap) {
    printf("malloc failed\n");
    exit(-1);
  }
  allocptr=heap;
  go();   // call into the generated code
 return 0;
}
