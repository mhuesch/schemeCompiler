...
import qualified L3.Helpers as L3H
import qualified L2.Helpers as L2H
...
compileE (L3.If v e1 e2) = do
    ...
    c_e1 <- compileE e1
    c_e2 <- compileE e2
    return . concat $ [[L2.ICjump (encodeT v) L2.Equal (L2.Tnum $ L2H.intToN 1) e2_label e1_label]
                      ,[L2.ILabel e1_label]
                      ,c_e1
                      ,[L2.ILabel e2_label]
                      ,c_e2]
...
compileD target isTail (L3.Binop biop v1 v2) = do
    ...
    liftM (addReturn isTail) $ case biop of
        L3.Add -> return [L2.IAssign tmpW (encodeS v1)
                        ...
                        ,L2.IArith tmpW L2.Sub (L2.Tnum $ L2H.intToN 1)
                        ,L2.IAssign target (L2.Sx $ L2.Xw tmpW)]
        L3.Sub -> return [L2.IAssign tmpW (encodeS v1)
                        ,L2.IArith tmpW L2.Sub (encodeT v2)
                        ,L2.IArith tmpW L2.Add (L2.Tnum $ L2H.intToN 1)
                        ,L2.IAssign target (L2.Sx $ L2.Xw tmpW)]
        L3.Mult -> do
            return $ [L2.IAssign tmpW (encodeS v1)
                     ,L2.IShiftN tmpW L2.ShiftRight (L2H.intToN 1)
                     ,L2.IAssign target (encodeS v2)
                     ,L2.IShiftN target L2.ShiftRight (L2H.intToN 1)
...
compileD target isTail (L3.Predicate pr v) = return . addReturn isTail $
    ...
    ,L2.IArith target L2.And (L2.Tnum $ L2H.intToN 1)]
    ++ case pr of
            L3.IsNum -> encodeInstrs target
            L3.IsA -> [L2.IArith target L2.Mult (L2.Tnum $ L2H.intToN (-2))
                     ,L2.IArith target L2.Add (L2.Tnum $ L2H.intToN 3)]

-- Must set return label
compileD target isTail (L3.Apply v vs) = do
    retLab <- newLab
    let callArityPNI = L2H.intToPNI $ length vs
    case isTail of
...
compileD target isTail (L3.NewArray v1 v2) = return . addReturn isTail $
    (setVars [v1,v2]) ++
    [L2.ICallRuntime L2.Allocate (L2H.intToPNI 2)
    ,L2.IAssign target raxS]
...
compileD target isTail (L3.NewTuple vs) = do
    ...
    let tmpW = variableToW tmp
        ...
        vSs = map encodeS vs
        body = (setVars [L3.Vnum . L3H.intToPNI . length $ vs, L3.Vnum $ L3H.intToPNI 1])
               ++ [L2.ICallRuntime L2.Allocate (L2H.intToPNI 2)
...
compileD target isTail (L3.Aref v1 v2) = do
    ...
    return . addReturn isTail $
        ...
        ,L2.IShiftN tmp1W L2.ShiftRight (L2H.intToN 1)
        ,L2.IReadMem tmp2W v1X (L2H.intToN 0)
        ,L2.ICjump (L2.Tx $ L2.Xw tmp1W) L2.LessThan (L2.Tx $ L2.Xw tmp2W) notBadLabel badLabel
        ,L2.ILabel notBadLabel
        ,L2.ICjump (L2.Tx $ L2.Xw tmp1W) L2.LessThan (L2.Tnum $ L2H.intToN 0) badLabel goodLabel
        ,L2.ILabel badLabel]
        ++
        (setVars [v1,v2])
        ++
        [L2.ICallRuntime L2.ArrayError (L2H.intToPNI 2)
        ,L2.ILabel goodLabel
        ,L2.IArith tmp1W L2.Mult (L2.Tnum $ L2H.intToN 8)
        ,L2.IArith tmp1W L2.Add (L2.Tx v1X)
        ,L2.IReadMem target (L2.Xw tmp1W) (L2H.intToN 8)]
...
compileD target isTail (L3.Aset v1 v2 v3) = do
    ...
    return . addReturn isTail $
        ...
        ,L2.IArith tmp1W L2.Mult (L2.Tnum $ L2H.intToN 8)
        ,L2.IArith tmp1W L2.Add (L2.Tx v1X)
        ,L2.IWriteMem (L2.Xw tmp1W) (L2H.intToN 8) (encodeS v3)
        ,L2.IAssign target (L2.Snum $ L2H.intToN 1)]

compileD target isTail (L3.Alen v) = return . addReturn isTail $
    [L2.IReadMem target (compileVX v) (L2H.intToN 0)]
    ++ encodeInstrs target

compileD target isTail (L3.Print v) = return . addReturn isTail $
    (setVars [v])
    ++
    [L2.ICallRuntime L2.Print (L2H.intToPNI 1)
    ,L2.IAssign target raxS]

compileD target isTail (L3.MakeClosure proc vars) = compileD target isTail (L3.NewTuple [L3.Vlab proc,vars])

compileD target isTail (L3.ClosureProc clos) = compileD target isTail (L3.Aref clos (L3.Vnum $ L3H.intToPNI 0))

compileD target isTail (L3.ClosureVars clos) = compileD target isTail (L3.Aref clos (L3.Vnum $ L3H.intToPNI 1))
...
encodeInstrs w = [L2.IArith w L2.Mult (L2.Tnum $ L2H.intToN 2)
                 ,L2.IArith w L2.Add (L2.Tnum $ L2H.intToN 1)]
...
encodeS (L3.Vnum (L3.PosNegInteger n)) = L2.Snum . L2H.intToN $ (2*(read n) + 1)
encodeS (L3.Vlab l) = L2.Slab $ compileLabel l

encodeT :: L3.V -> L2.T
encodeT (L3.Vx x) = L2.Tx $ compileXX x
encodeT (L3.Vnum (L3.PosNegInteger n)) = L2.Tnum . L2H.intToN $ (2*(read n) + 1)
